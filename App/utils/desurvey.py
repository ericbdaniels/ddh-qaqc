...
def get_survey_interp(row, survey):
    ...
    az, dip = interp_survey(
        top.AZIMUTH, top.DIP, bottom.AZIMUTH, bottom.DIP, length, dist
    )
    return int(dhid), az, dip, assay_depth


def desurvey_assay(assay, survey, collar):
    interp = assay.apply(
        get_survey_interp, args=(survey,), axis=1, result_type="expand"
    )
    interp.rename(
        columns=dict(zip(interp.columns, ["DHID", "AZIMUTH", "DIP", "DEPTH"])),
        inplace=True,
    )
    desurveyed = desurvey(interp, collar)
    return desurveyed.drop("DHID", axis=1).join(assay)